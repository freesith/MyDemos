...
public class Sticker {
     ...
     * 获取关闭按钮中心点
     */
    public PointF getCloseMidPoint(Matrix matrix) {
        PointF point = new PointF();
        float[] points = StickerUtils.getBitmapPoints(srcImage, matrix);
        float x1 = points[0];
        float y1 = points[1];
     ...
     * @return
     */
    public float getSpaceRawRotation(MotionEvent event, PointF imageMidPoint) {
        double deltaX = event.getRawX() - imageMidPoint.x;
        double deltaY = event.getRawY() - imageMidPoint.y;
        double radians = Math.atan2(deltaY, deltaX);
        return (float) Math.toDegrees(radians);
    }
     ...
     * @return
     */
    public float getMultiTouchRotation(MotionEvent event) {
    ...
    public float getSingleTouchRawDistance(MotionEvent event, PointF closeMidPoint) {
        float x = event.getRawX() - closeMidPoint.x;
        float y = event.getRawY() - closeMidPoint.y;
        return (float) Math.sqrt(x * x + y * y);
